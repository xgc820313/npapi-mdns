...
int avahi_service_browser(void) {
    ...
    if (!(simple_poll = avahi_simple_poll_new())) {
        DBGLOG(LOG_ERR, "avahi_service_browser: can't create simple_poll");
        goto fail;
    }
    ...
    if (!client) {
    	DBGLOG(LOG_ERR, "avahi_service_browser: can't create client");
        goto fail;
    }
    ...
    if (!(sb = avahi_service_browser_new(client, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, "_http._tcp", NULL,(AvahiLookupFlags) 0, browse_callback, client))) {
    	DBGLOG(LOG_ERR, "avahi_service_browser: can't create service browser");
        goto fail;
    }
